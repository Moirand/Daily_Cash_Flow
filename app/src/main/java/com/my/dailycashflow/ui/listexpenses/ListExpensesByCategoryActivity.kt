...
class ListExpensesByCategoryActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.getExpensesByCategory(id).observe(this){
            showExpenseData(it)
        }
    }

    private fun setUpRecyclerView(){
        rvExpenses = findViewById(R.id.recyclerview_list_expenses_by_category)
        adapterExpenses = ListCashFlowAdapter()

        rvExpenses.layoutManager = LinearLayoutManager(this)
        rvExpenses.adapter = adapterExpenses
